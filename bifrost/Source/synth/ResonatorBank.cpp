...
float ResonatorBank::process(float input, const TimbreModel& model, float modelTimeSeconds, const VoiceRenderParameters& params)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        const float q = std::clamp(model.resonators.q[static_cast<size_t>(i)], 0.4f, 16.0f);
        const float g = std::clamp(model.resonators.gains[static_cast<size_t>(i)].sample(modelTimeSeconds) * params.metal, 0.0f, 2.0f);
