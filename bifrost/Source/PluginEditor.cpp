...
BifrostAudioProcessorEditor::BifrostAudioProcessorEditor(BifrostAudioProcessor& p)
    : AudioProcessorEditor(&p), audioProcessor(p), sampleDropTarget(p), waveformView(p), stateMapView(p), adsrEnvelopeView(p)
{
    ...
    title.setFont(BifrostLookAndFeel::logoFont(21.0f).withExtraKerningFactor(0.12f));
    ...
    for (auto* label : { &macrosLabel, &envelopeLabel, &performanceLabel, &modeLabel, &qualityLabel })
    ...
    for (auto* slider : { &bodySlider, &airSlider, &metalSlider, &brightnessSlider, &motionSlider, &inertiaSlider, &mutationSlider, &transientSlider,
                          &attackSlider, &decaySlider, &sustainSlider, &releaseSlider, &adsrCurveSlider, &velocitySlider, &panSlider, &stereoWidthSlider,
                          &stereoReconstructSlider, &phaseOffsetSlider, &phaseRandomSlider, &timeSyncSlider, &rootRandomSlider, &randomDirectionSlider, &loopStartSlider, &loopEndSlider,
                          &unisonVoicesSlider, &unisonDetuneSlider })
        addAndMakeVisible(*slider);

    configureRootOverrideSlider();
    modeBox.addItemList(juce::StringArray{ "One-Shot", "Sustain", "Harmonizer" }, 1);
    ...
    for (auto* slider : { &bodySlider, &airSlider, &metalSlider, &brightnessSlider, &motionSlider, &inertiaSlider, &mutationSlider, &transientSlider,
                          &velocitySlider, &stereoWidthSlider, &stereoReconstructSlider, &phaseOffsetSlider, &phaseRandomSlider, &timeSyncSlider, &adsrCurveSlider, &rootRandomSlider,
                          &randomDirectionSlider, &loopStartSlider, &loopEndSlider })
        slider->textFromValueFunction = normalizedText;
    panSlider.textFromValueFunction = [](double value)
    {
        if (std::abs(value) < 0.005)
            return juce::String("C");
        return juce::String(value < 0.0 ? "L " : "R ") + juce::String(static_cast<int>(std::round(std::abs(value) * 100.0))) + "%";
    };
    ...
    modeAttachment = std::make_unique<ComboBoxAttachment>(audioProcessor.parameters(), "mode", modeBox);
    qualityAttachment = std::make_unique<ComboBoxAttachment>(audioProcessor.parameters(), "quality", qualityBox);
    rootOverrideAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "root_override_hz", rootOverrideSlider);
    bodyAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "body", bodySlider);
    airAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "air", airSlider);
    metalAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "metal", metalSlider);
    brightnessAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "brightness", brightnessSlider);
    motionAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "motion", motionSlider);
    inertiaAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "inertia", inertiaSlider);
    mutationAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "mutation", mutationSlider);
    transientAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "transient", transientSlider);
    attackAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "attack", attackSlider);
    decayAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "decay", decaySlider);
    sustainAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "sustain", sustainSlider);
    releaseAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "release", releaseSlider);
    adsrCurveAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "adsr_curve", adsrCurveSlider);
    velocityAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "velocity_sensitivity", velocitySlider);
    panAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "pan", panSlider);
    stereoWidthAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "stereo_width", stereoWidthSlider);
    stereoReconstructAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "stereo_reconstruction", stereoReconstructSlider);
    phaseOffsetAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "phase_offset", phaseOffsetSlider);
    phaseRandomAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "phase_random", phaseRandomSlider);
    timeSyncAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "time_sync", timeSyncSlider);
    rootRandomAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "root_random", rootRandomSlider);
    randomDirectionAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "random_direction", randomDirectionSlider);
    loopStartAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "loop_start", loopStartSlider);
    loopEndAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "loop_end", loopEndSlider);
    unisonVoicesAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "unison_voices", unisonVoicesSlider);
    unisonDetuneAttachment = std::make_unique<SliderAttachment>(audioProcessor.parameters(), "unison_detune", unisonDetuneSlider);
    ...
    {
        audioProcessor.analysis().cancel();
    };
...
void BifrostAudioProcessorEditor::configureSlider(juce::Slider& slider, const juce::String& name)
{
    slider.setName(name);
...
void BifrostAudioProcessorEditor::configureAdsrSlider(juce::Slider& slider, const juce::String& name, bool secondsValue)
{
    slider.setName(name);
...
void BifrostAudioProcessorEditor::configureLabel(juce::Label& label, const juce::String& text, float alpha)
{
    label.setText(text.toUpperCase(), juce::dontSendNotification);
    label.setJustificationType(juce::Justification::centredLeft);
    label.setColour(juce::Label::textColourId, BifrostLookAndFeel::text().withAlpha(alpha));
    label.setFont(BifrostLookAndFeel::uiFont(11.5f, true).withExtraKerningFactor(0.10f));
    addAndMakeVisible(label);
}

void BifrostAudioProcessorEditor::placeLabeledKnob(juce::Rectangle<int> bounds, juce::Slider& slider, juce::Label& label)
{
    label.setJustificationType(juce::Justification::centred);
...
void BifrostAudioProcessorEditor::enforceLoopRange(juce::Slider& movedSlider)
{
    ...
    {
        loopStartSlider.setValue(std::max(loopStartSlider.getMinimum(), end - minLoopGap), juce::sendNotificationSync);
    }
    else if (&movedSlider == &loopEndSlider && end < start + minLoopGap)
    {
        loopEndSlider.setValue(std::min(loopEndSlider.getMaximum(), start + minLoopGap), juce::sendNotificationSync);
    }
}
...
void BifrostAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.setColour(BifrostLookAndFeel::panel().withAlpha(BifrostLookAndFeel::outerBoxFillOpacity));
    g.fillRoundedRectangle(outer, 5.0f);
    g.setColour(BifrostLookAndFeel::panelEdge().withAlpha(BifrostLookAndFeel::outerBoxBorderOpacity));
    g.drawRoundedRectangle(outer, 5.0f, 1.0f);

    for (auto section : { rootSectionBounds, envelopeSectionBounds, performanceSectionBounds, timbreSectionBounds })
    {
        if (!section.isEmpty())
            BifrostLookAndFeel::fillPanel(g, section.toFloat(), 5.0f);
    }

    BifrostLookAndFeel::drawBifrostGlyph(g, juce::Rectangle<float>(24.0f, 24.0f, 34.0f, 34.0f), 0.88f);
}
...
void BifrostAudioProcessorEditor::resized()
{
    ...
    title.setBounds(titleRow.removeFromLeft(202).reduced(0, 9));
    ...
    auto placeAdsr = [&adsrRow, adsrWidth](juce::Slider& slider, juce::Label& label)
    {
    ...
    if (!loopAndCurve.isEmpty())
    {
        ...
        placeLabeledKnob(loopAndCurve.removeFromLeft(quarter).reduced(7), adsrCurveSlider, adsrCurveLabel);
        placeLabeledKnob(loopAndCurve.removeFromLeft(quarter).reduced(7), loopStartSlider, loopStartLabel);
        placeLabeledKnob(loopAndCurve.removeFromLeft(quarter).reduced(7), loopEndSlider, loopEndLabel);
    ...
    auto placePerformance = [this, &performance, performanceColW](juce::Slider& slider, juce::Label& label)
    {
        placeLabeledKnob(performance.removeFromLeft(performanceColW).reduced(12, 2), slider, label);
    };
